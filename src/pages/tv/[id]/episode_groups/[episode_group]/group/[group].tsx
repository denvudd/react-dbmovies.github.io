...
export const getServerSideProps: GetServerSideProps<{
  ...
  const { id, episode_group, group } = context.query;
  ...
  if ("id" in response && response.id) {
    return {
      props: { data: response as TVEpisodeGroupDetailsApiResponse, id, groupId: group },
    };
  } else {
    return {
      redirect: {
...
const TVEpisodeGroupEpisodesPage: React.FC<TVSingleEpisodeGroupPageProps> = ({
  data,
  id,
  groupId
}) => {
  ...
  return (
    <>
      ...
      <div className="content-with-aside" style={{ paddingBottom: "30px" }}>
        <DetailLayout>
          <TVEpisodeGroupEpisodesBlock seriesId={id} episodeGroup={data} groupId={groupId} />
        </DetailLayout>
      </div>
    </>
  );
...
export default TVEpisodeGroupEpisodesPage;
