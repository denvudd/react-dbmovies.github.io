...
type TVLogosPageApiResponse = TVDetails & {
  images: TVImagesApiResponse;
};

export interface TVLogosPageProps {
  data: TVLogosTransformedData;
}

interface TVLogosTransformedData {
  name: string;
  images: TVImagesApiResponse["logos"];
...
export const getServerSideProps: GetServerSideProps<{
  ...
  const response: TVLogosPageApiResponse | ApiError = await res.json();
  ...
  if ("id" in response && response.id) {
    ...
    const transformedData: TVLogosTransformedData = {
      name,
      images: images.logos,
      first_air_date,
      poster_path,
...
const TVPostersPage: React.FC<TVLogosPageProps> = ({ data }) => {
  ...
  return (
    <>
      ...
      <div className="app-container content-with-aside panel-details">
        <DetailLayout>
          <TVImagesBlock images={images} name={name} type="logo" />
        </DetailLayout>
      </div>
    </>
  );
