...
import TVImagesBlock from "@/components/blocks/tv/TVImagesBlock/TVImagesBlock";
...
type TVPostersPageApiResponse = TVDetails & {
  images: TVImagesApiResponse;
};

export interface TVPostersPageProps {
  data: TVPostersTransformedData;
}

interface TVPostersTransformedData {
  name: string;
  images: TVImagesApiResponse["posters"];
...
export const getServerSideProps: GetServerSideProps<{
  ...
  const res = await fetch(
    `https://api.themoviedb.org/3/tv/${id}?append_to_response=images&language=uk-UA&include_image_language=en&api_key=684e3f73d1ca0e692a3016c028aabf72`
  );
  const response: TVPostersPageApiResponse | ApiError = await res.json();
  ...
  if ("id" in response && response.id) {
    ...
    const transformedData: TVPostersTransformedData = {
      name,
...
const TVPostersPage: React.FC<TVPostersPageProps> = ({ data }) => {
  ...
  return (
    <>
      ...
      <div className="app-container content-with-aside panel-details">
        <DetailLayout>
          <TVImagesBlock images={images} name={name} type="poster" />
        </DetailLayout>
      </div>
    </>
  );
