...
const KeywordMoviePage = () => {
  const router = useRouter();
  const [initLoading, setInitLoading] = React.useState(true);
  const [initPage, setInitPage] = React.useState(1);
  const [loading, setLoading] = React.useState(false);
  const [data, setData] = React.useState<ListMovie[]>([]);
  const [list, setList] = React.useState<ListMovie[]>([]);
  const [mode, setMode] = React.useState<"col" | "row">("col");
  const [getSearch, { data: searchResults }] = useLazyGetMovieDiscoverQuery();
  const [
    getKeywordName,
          ...
          total={
            <Dropdown menu={{ items }} trigger={["click"]}>
              <span style={{ cursor: "pointer" }}>
                {`${searchResults ? searchResults.total_results : "0"} фільмів`}
                <DownOutlined
                  style={{
                    fontSize: "0.8em",
                    marginLeft: "5px",
...
export default KeywordMoviePage;
