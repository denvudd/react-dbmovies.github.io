...
import MovieReviewsBlock from "@/components/blocks/movie/MovieReviewsBlock/MovieReviewsBlock";
...
export interface MovieReviewsPageProps {
  data: MovieReviewsTransformedData;
}

interface MovieReviewsTransformedData {
...
export const getServerSideProps: GetServerSideProps<{
  ...
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}?append_to_response=reviews&api_key=684e3f73d1ca0e692a3016c028aabf72`
  );
  const response: MovieReviewsPageApiResponse | ApiError = await res.json();
  ...
  if ("id" in response && response.id) {
    ...
    const transformedData: MovieReviewsTransformedData = {
  ...
  return (
    <>
      ...
      <div className="app-container content-with-aside panel-details">
        <DetailLayout>
          <MovieReviewsBlock reviews={reviews} />
        </DetailLayout>
      </div>
    </>
  );
