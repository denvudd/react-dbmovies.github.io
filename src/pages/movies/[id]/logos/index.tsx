...
type MovieLogosPageApiResponse = MovieDetails & {
  images: MovieImagesApiResponse;
};

export interface MovieLogosPageProps {
  data: MovieLogosTransformedData;
}

interface MovieLogosTransformedData {
  title: string;
  images: MovieImagesApiResponse["logos"];
  release_date: string;
...
export const getServerSideProps: GetServerSideProps<{
  ...
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}?append_to_response=images&language=uk-UA&include_image_language=en&api_key=684e3f73d1ca0e692a3016c028aabf72`
  );
  const response: MovieLogosPageApiResponse | ApiError = await res.json();
  ...
  if ("id" in response && response.id) {
    ...
    const transformedData: MovieLogosTransformedData = {
      title,
      images: images.logos,
      release_date,
      poster_path,
...
const MovieLogosPage: React.FC<MovieLogosPageProps> = ({ data }) => {
  const { title, images, release_date, poster_path, overview, id } = data;
  ...
  return (
    <>
      ...
      <div className="app-container content-with-aside panel-details">
        <DetailLayout>
          <MovieImagesBlock images={images} title={title} type="logo" />
        </DetailLayout>
      </div>
    </>
  );
