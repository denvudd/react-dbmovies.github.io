...
export interface MovieBackdropsPageProps {
  data: MovieBackdropsTransformedData;
}

interface MovieBackdropsTransformedData {
  title: string;
  images: MovieImagesApiResponse["backdrops"];
  release_date: string;
...
export const getServerSideProps: GetServerSideProps<{
  ...
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}?append_to_response=images&language=uk-UA&include_image_language=null&api_key=684e3f73d1ca0e692a3016c028aabf72`
  );
  const response: MovieBackdropsPageApiResponse | ApiError = await res.json();
  ...
  if ("id" in response && response.id) {
    ...
    const transformedData: MovieBackdropsTransformedData = {
      title,
...
const MovieBackdropsPage: React.FC<MovieBackdropsPageProps> = ({ data }) => {
  ...
  return (
    <>
      ...
      <div className="app-container content-with-aside panel-details">
        <DetailLayout>
          <MovieImagesBlock images={images} title={title} type="backdrop" />
        </DetailLayout>
      </div>
    </>
  );
