...
import MovieReleasesBlock from "@/components/blocks/movie/MovieReleasesBlock/MovieReleasesBlock";
...
export interface MovieReleasesPageProps {
  data: MovieReleasesTransformedData;
}

interface MovieReleasesTransformedData {
...
export const getServerSideProps: GetServerSideProps<{
  ...
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}?append_to_response=release_dates&language=uk-UA&api_key=684e3f73d1ca0e692a3016c028aabf72`
  );
  const response: MovieReleasesPageApiResponse | ApiError = await res.json();
  ...
  if ("id" in response && response.id) {
    ...
    const transformedData: MovieReleasesTransformedData = {
...
const MovieReleasesPage: React.FC<MovieReleasesPageProps> = ({ data }) => {
  const { poster_path, title, overview, release_dates, release_date, id } =
    data;
        ...
        <ListLayout siderTheme="light">
          {{
            sidebar: (
              <DetailsSider
                title={"Дати виходу"}
                totalCount={release_dates.length}
                items={release_dates}
                averageColor={averageColor}
                isBackdropLight={isBackdropLight}
              />
            ),
            mainContent: <MovieReleasesBlock countries={release_dates} />,
          }}
        </ListLayout>
      </div>
    </>
